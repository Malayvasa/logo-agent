...
export async function vectorize(
  ...
  const createResult = await executeTool("BROWSER_TOOL_CREATE_TASK", {
    task: [
      ...
      `Before vectorizing, configure these settings in the Advanced Options section:`,
      `  Quality Enhancement tab: Anti-aliasing Off, Noise Reduction Off, Upscaling 400%`,
      `  Vectorization tab: Roundness to the second option (medium), Minimum Area 20px², Overlap Low, Circle Detection On`,
      `  Output Size tab: Width 64 pixels, Height 64 pixels (fixed ratio)`,
      `  ≤15KB GT tab: enable "SVG minimization, layer & path splitting (15KB)" checkbox`,
